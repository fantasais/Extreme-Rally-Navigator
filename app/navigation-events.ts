...
export const createNavigationState = (): NavigationState => ({
  ...
  goodFixes: 0,
  ready: false,
  recoveryReason: "INITIAL_ACQUISITION",
  transitions: [],
});
...
export function advanceNavigation(
  definitions: NavigationEvent[],
  state: NavigationState,
  fix: EventFix,
): NavigationState {
  if (
    !Number.isFinite(fix.timestamp) ||
    fix.timestamp <= state.lastTimestamp
  ) {
    return state;
  }
  ...
  if (
    ...
    fix.ambiguous ||
    (fix.speedMps > 2.5 &&
      fix.headingError !== undefined &&
      (!Number.isFinite(fix.headingError) ||
        fix.headingError > 75))
  ) {
  ...
  if (fix.rejoined) {
    ...
    const allowance = Math.max(
      12,
      Math.max(fix.speedMps, previous.speedMps) *
        dt *
        1.7 +
        Math.max(fix.accuracy, previous.accuracy) +
        5,
    );

    if (
      dt > 3.5 ||
      travel < -Math.max(5, fix.accuracy) ||
      travel > allowance
    ) {
      invalidate(
        dt > 3.5 ? "GPS_GAP" : "POSITION_REALIGNMENT",
      );
      previous = null;
    }
  }
  ...
  for (const event of definitions) {
    ...
    const margin = Math.max(
      2,
      Math.min(6, fix.accuracy * 0.25),
    );

    const phase: EventPhase =
      fix.distance >= event.distance - margin
        ? "EXECUTING"
        : event.distance - fix.distance <=
            Math.max(event.lead, fix.speedMps * 8)
          ? "APPROACHING"
          : "UPCOMING";

    const before =
      fix.distance <= event.passDistance
        ? fix
        : old?.before;
    ...
    if (
      !next.ready ||
      fix.distance < event.passDistance + margin
    ) {
      continue;
    }
    ...
    const bracket =
      ...
      fix.distance - before.distance >
        Math.max(2, fix.accuracy * 0.25) &&
      Math.max(fix.speedMps, before.speedMps) >= 0.8;

    if (bracket) {
      const ratio =
        (event.passDistance - before.distance) /
        (fix.distance - before.distance);
      ...
      previous &&
      previous.distance >= event.passDistance &&
      fix.distance - previous.distance >= 1 &&
      fix.speedMps >= 0.8
    ) {
      next.events[event.id] = {
        phase: "PASSED",
        reason: "PASS_TIME_UNCONFIRMED",
      };
...
export function displayProgress(
  ...
  const seconds =
    reliable && age >= 0 && age <= 1500
      ? Math.min(age / 1000, 1.1)
      : 0;
  ...
  return Math.max(
    ...
    Math.min(
      totalDistance,
      confirmedDistance +
        (Math.max(0, speedKph) / 3.6) * seconds,
    ),
  );
}

export function zoneFromEvents(
  state: NavigationState,
  start: number,
  finish: number,
) {
  ...
  if (eventFinished(fz)) {
    return {
      ...
      enteredAt:
        dz?.phase === "PASSED" ? dz.at : undefined,
      finishedAt:
        fz?.phase === "PASSED" ? fz.at : undefined,
      uncertain:
        dz?.phase !== "PASSED" ||
        fz?.phase !== "PASSED" ||
        dz.at === undefined ||
